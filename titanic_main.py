...
# Join train and test datasets in order to obtain the
# same number of features during categorical conversion
...
# dataset["Fare"] = dataset.group(["Embarked", "Pclass"], as_index=False)["Fare"].transform(
#     lambda x: x.fillna(x.mean()))
# Apply log to Fare to reduce skewness distribution
dataset['Fare'] = dataset['Fare'].map(lambda x: np.log(x) if x > 0 else 0)
dataset['Fare'] = dataset['Fare'].fillna(dataset[((dataset['Pclass'] == 3) &
                                                  (dataset['Embarked'] == 'S') &
                                                  (dataset['Cabin'] == 'U'))]['Fare'].mean())
# feature engineering
# explore Name feature
# for data in combine_drop_data:
# data["Title"] = data.Name.str.extract("([A-Za-z]+)\.", expand=False)

# Get Title from Name
dataset['Title'] = dataset['Name'].map(lambda x: x.split(',')[1].split('.')[0].strip())
royalty = ['the Countess', 'Don', 'Lady', 'Sir', 'Dona']
...
# convert Title into categorical value dataset["Title"] = dataset["Title"].map({"Mr": 0,
# "Mrs": 1, "Miss": 2, "Master": 3, "Aristocrat": 4, "Rev": 5}) Filling missing value of Age
# sub_data = dataset[dataset["Age"].notnull()] sub_x = sub_data[["SibSp", "Parch", "Pclass",
# "Title"]] sub_y = sub_data["Age"] x_sub_train, x_sub_test, y_sub_train, y_sub_test =
# train_test_split(sub_x, sub_y, test_size=0.2, random_state=0) Age_model = LinearRegression(
# copy_X=True, fit_intercept=True, n_jobs=-1, normalize=False) Age_model.fit(x_sub_train,
# y_sub_train) print(mean_squared_error(y_subtest, Age_model.predict(x_subtest))) Index of NaN
# age rows index_NaN_age = list(dataset["Age"][dataset["Age"].isnull()].index) for i in
# index_NaN_age: dataset.loc[:, ['Age']].iloc[i] = Age_model.predict(dataset[["SibSp", "Parch",
# "Pclass", "Title"]])[i] dataset.loc[dataset["Age"] < 15, "Age"] = 0 dataset.loc[(dataset["Age"]
# < 55) & (dataset["Age"] >= 15), "Age"] = 1 dataset.loc[dataset["Age"] >= 55, "Age"] = 2
# dataset["Age"] = dataset["Age"].astype("category") dataset = pd.get_dummies(dataset, columns =
# ["Age"], prefix="A")
...
FemChildDf = dataset[((dataset['Sex'] == 'female') | (dataset['Age'] <= 12)) &
                     (dataset['FamilyNum'] >= 2)]
...
dataset.loc[((dataset['Survived'].isnull()) & (dataset['Surname'].isin(MSurNamDict)) &
             (dataset['Sex'] == 'male')), 'Age'] = 5
dataset.loc[((dataset['Survived'].isnull()) & (dataset['Surname'].isin(MSurNamDict)) &
             (dataset['Sex'] == 'male')), 'Sex'] = 'female'

# 对数据集中这些姓氏的女性及儿童的数据进行修正：1、性别改为男；2、年龄改为60。
dataset.loc[((dataset['Survived'].isnull()) & (dataset['Surname'].isin(FCSurNamDict)) &
             ((dataset['Sex'] == 'female') | (dataset['Age'] <= 12))), 'Age'] = 60
dataset.loc[((dataset['Survived'].isnull()) & (dataset['Surname'].isin(FCSurNamDict)) &
             ((dataset['Sex'] == 'female') | (dataset['Age'] <= 12))), 'Sex'] = 'male'
# 人工筛选
...
# classifiers.append(ExtraTreesClassifier(oob_score=True, bootstrap=True,
...
# #################################################################################################
# classifiers.append(XGBClassifier((use_label_encoder=False, random_state=RANDOM_STATE,
#                                   learning_rate=0.06, max_depth=4, min_child_weight=1.1,
#                                   n_estimators=80, reg_alpha=0.015, reg_lambda=0.1)))
# classifiers.append(AdaBoostClassifier((DecisionTreeClassifier(random_state=RANDOM_STATE,
#                                        max_depth=6, min_samples_split=10),
#                                        random_state=RANDOM_STATE, learning_rate=0.009778,
#                                        n_estimators=40)))
# classifiers.append(ExtraTreesClassifier((oob_score=True, bootstrap=True,
#                                          random_state=RANDOM_STATE, max_depth=8, max_features=9,
#                                          min_samples_split=3, n_estimators=30)))
# classifiers.append(GradientBoostingClassifier((random_state=RANDOM_STATE, learning_rate=0.09556,
#                                                max_depth=3, min_samples_leaf=5,
#                                                min_samples_split=0.015, n_estimators=160)))
# classifiers.append(KNeighborsClassifier(n_neighbors=10, weights='uniform'))
# classifiers.append(LogisticRegression((random_state=RANDOM_STATE, C=0.7334,
#                                        class_weight=None, solver='liblinear')))
# classifiers.append(MultinomialNB(alpha=4.7368))
# classifiers.append(SVC((probability=True, random_state=RANDOM_STATE,
#                         C=0.6, gamma=0.06, kernel='rbf')))
# #################################################################################################
#
# cv_results = []
...
#                        "Algorithm": ["XGBClassifier", "AdaBoost", "RandomForest", "ExtraTrees",
#                                      "GradientBoosting", "KNeighboors", "LogisticRegression",
#                                      "MultinomialNB", "SGDClassifier", "SVC"]})
####################################################################################################
# # cv_res = pd.DataFrame({"CrossValMeans": cv_means, "CrossValerrors": cv_std,
#                          "Algorithm": ["XGBClassifier", "AdaBoost", "ExtraTrees",
#                                        "GradientBoosting", "KNeighboors", "LogisticRegression",
#                                        "MultinomialNB", "SVC"]})
####################################################################################################
...
####################################################################################################
# essembles = pd.DataFrame({"XGBC": preds[0], "AdaBoost": preds[1], "RandomForest": preds[2],
#                           "ET": preds[3], "GBC": preds[4], "KNN": preds[5], "LR": preds[6],
#                           "MLNB": preds[7], "SGDClassifier": preds[8], "SVC": preds[9]})
####################################################################################################
...
# adasboost
# ada_best, ada_score, ada_params = search_best_AdaBoostClassifier(X_train, Y_train, k_fold, RANDOM_STATE)
...
# names_classifiers = [("XGBC", classifiers[0]), ("AdaBoosting", classifiers[1]),
#                      ("ExtraTrees", classifiers[2]), ("GradientBoosting", classifiers[3])]
#
# nclassifier = 0
...
#                         x=classifier.feature_importances_[indices][:10],
...
#
# plt.show()
# estimators=[("XGBC", classifiers[0]), ("AdaBoosting", classifiers[1]),
#             ("ExtraTrees", classifiers[2]), ("GradientBoosting", classifiers[3]),
#             ("KNN", classifiers[4]), ('LR', classifiers[5]), ('MLNB', classifiers[6]),
#             ('SVC', classifiers[7])]
# votingC = VotingClassifier(estimators=estimators, voting='hard', n_jobs=-1)
#
...
# 查看模型ROC曲线
# 求出测试数据模型的预测值
predict_y = LR_best.predict(X_train).astype(int)
# 画图
# Compute ROC curve and ROC area for each class
fpr, tpr, threshold = roc_curve(Y_train, predict_y)  # 计算真正率和假正率
...
print('GradientBoostingClassifier模型混淆矩阵为\n',
      confusion_matrix(Y_train.astype(int).astype(str), predict_y.astype(str)))
# TitanicGBSmodle
final_pre_y = LR_best.predict(test)
final_pre_y = final_pre_y.astype(int)

# 导出预测结果
GBCpreResultDf = pd.DataFrame()
GBCpreResultDf['PassengerId'] = dataset['PassengerId'][dataset['Survived'].isnull()]
GBCpreResultDf['Survived'] = final_pre_y
